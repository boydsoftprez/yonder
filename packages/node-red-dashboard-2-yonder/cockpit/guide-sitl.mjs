...
import {unpackInstruments} from '../src/ui/cockpit/instrumentation-client.mjs';
...
let initial=await state();const heartbeatDeadline=Date.now()+15000;while(initial.telemetry.armed===null&&Date.now()<heartbeatDeadline){await new Promise(r=>setTimeout(r,250));initial=await state()}assert.equal(initial.telemetry.armed,false);assert(initial.mission.items.every(item=>item.seq===0),'Use a fresh disposable simulator without an authored mission');
...
try{
 ...
 const instrumentResponse=await fetch(new URL('/cockpit/api/instruments',base));assert(instrumentResponse.ok);const instrumentText=await instrumentResponse.text();const instruments=unpackInstruments(JSON.parse(instrumentText));assert.equal(instruments.generation,identity);assert(instruments.fields['flight.airborneSeconds'].value>0);assert(instruments.fields['flight.autoSeconds'].value>0);assert(instruments.fields['flight.bootSeconds'].value>0);assert(instruments.fields['fc.loadPercent'].value!==null);records.push({name:'Live instrumentation and observed flight time',bytes:new TextEncoder().encode(instrumentText).length,fieldCount:Object.keys(instruments.fields).length,fields:Object.fromEntries(['flight.bootSeconds','flight.armedSeconds','flight.airborneSeconds','flight.autoSeconds','flight.vtolState','flight.landedState','fc.loadPercent'].map(key=>[key,instruments.fields[key]]))});console.log('Live instrumentation and observed flight time');
