...
const drawerLayout=new URLSearchParams(location.search).get('layout')==='drawer'
...
let previous=null,id=0,auth=true,recallCount=0,fixtureMode='FPV',fixtureRoll=28.8
const aimCalls=[];window.__aimFixture={calls:aimCalls}
...
window.fetch=async(url,options={})=>{
 ...
 if(String(url).endsWith('/aim')){const b=JSON.parse(options.body);aimCalls.push(b);if(b.op==='slew' && b.roll){fixtureRoll+=b.roll*.1;hydrate()}activity.value=`Fixture aim: ${JSON.stringify(b)}`;if(b.op==='issue-recall')recallCount=0;if(b.op==='mode'){fixtureMode=['Free','FPV','Follow'][b.mode];hydrate()}return new Response(JSON.stringify({accepted:true,grant:{gesture:'fixture',credential:'fixture',deadline:Date.now()+500},next:{gesture:'fixture',credential:'fixture',deadline:Date.now()+500},arrived:b.op==='recall'&&++recallCount>=5,name:'Fixture position',rate:{pan:0,tilt:0}}),{status:200})}
...
function hydrate(){
 ...
 store.state.data.messages.aim={payload:{state:'present',url:'/video/fixture/aim',generation:1,positionFrame:'handle',presets:fixturePresets,modeHelp:'FPV follows the handle on all axes.',recentreLabel:'Recenter in Follow',pan:12.4,tilt:-5.2,roll:fixtureRoll,rollControl:{available:fixtureMode==='FPV'&&scenario.value!=='Roll unavailable',reason:scenario.value==='Roll unavailable'?'Roll control has not been verified for this camera.':fixtureMode==='FPV'?null:'Choose FPV mode to control roll.',maxRate:120},bounds:null,maxRate:120,mode:fixtureMode,modes:['Free','FPV','Follow'],inhibited:null,imageDirection:'identity'}}
}
...
const app=createApp({setup:()=>()=>h('main',{id:'nrdb-page-page-camera'},[
 h('section',{class:'fixture-tools'},[h('strong','Camera workflow fixture'),h('span','No real device connections'),h('label',['Scenario ',h('select',{'aria-label':'Scenario',value:scenario.value,onChange:e=>choose(e.target.value)},['Running','Stopped','Starting','Failed','Unavailable','Camera unavailable','Expired session','No camera','Roll unavailable','RTSP active','RTSP waiting','RTSP unavailable','RTSP congestion'].map(value=>h('option',value)))]),h('label',['Theme ',h('select',{'aria-label':'Theme',value:theme.value,onChange:e=>{theme.value=e.target.value;document.querySelector('#theme').href=`/theme.${theme.value}.css`}},['day','night'].map(value=>h('option',value)))]),h('label',[h('input',{type:'checkbox',checked:reject.value,onChange:e=>{reject.value=e.target.checked}}),' Reject next Apply'])]),
 h('div',{class:['fixture-grid',drawerLayout?'fixture-workspace':''],key:epoch.value},[h('section',{class:['fixture-picture',drawerLayout?'nrdb-ui-group':''],id:drawerLayout?'nrdb-ui-group-group-cam-picture':undefined},[h(Picture,{id:'picture',props:{path:'cam3-preview'}})]),drawerLayout?h('section',{class:'nrdb-ui-group',id:'nrdb-ui-group-group-cam-aim'},[h('div',{class:'v-card'},[h('div',{class:'v-card-text nrdb-ui-group-content'},[h(Aim,{id:'aim',props:{collapsible:true}})])])]):h('section',[h(Aim,{id:'aim',props:{}})])]),h('section',{class:'fixture-deck',key:'deck-'+epoch.value},[h(Deck,{id:'deck',props:{}})]),h('p',{role:'status'},activity.value)
])})
