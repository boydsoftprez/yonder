  ...
  <section class="y-maint" aria-label="Network diagnostics">
    ...
    <form @submit.prevent="run">
      <div class="y-maint__fields">
        <label>Tool<select v-model="tool" :disabled="busy"><option value="internet-speed">Internet speed test</option><option value="ping">Ping</option><option value="traceroute">Traceroute</option><option value="route">Route lookup</option><option value="bandwidth">Advanced bandwidth · iperf3</option></select></label>
        <label v-if="tool !== 'internet-speed'">{{ tool === 'bandwidth' ? 'iperf3 server hostname or IP' : 'Hostname or IP address' }}<input v-model.trim="host" required maxlength="253" :disabled="busy" placeholder="1.1.1.1 or example.com" autocapitalize="none" spellcheck="false"></label>
      ...
      <p v-if="tool === 'internet-speed'" class="y-maint__muted">Test this device's internet download and upload speeds using Cloudflare. No server setup is needed. Uses up to 27 MB of test data, plus protocol overhead, and shares capacity with video and telemetry. Results are quick, single-connection HTTPS estimates.</p>
      ...
      <template v-if="tool === 'bandwidth'">
        ...
        <p class="y-maint__muted">About {{ (seconds * mbps / 8).toFixed(1) }} MB of test payload, plus protocol overhead.</p>
      </template>
      ...
      <div class="y-maint__actions"><button type="submit" class="y-maint__primary" :disabled="busy || (tool !== 'internet-speed' && !host)">{{ busy ? 'Running…' : tool === 'internet-speed' ? 'Start internet speed test' : 'Run test' }}</button><button type="button" :disabled="!job || job.status !== 'running' || submitting" @click="cancel">Cancel</button><button type="button" :disabled="busy || !job" @click="clear">Clear</button><button type="button" :disabled="!job" @click="download">Save output</button></div>
    </form>
    <p v-if="error" class="y-maint__error" role="alert">{{ error }}</p>
    <section v-if="job?.internetSpeed" class="y-diag__speed" aria-label="Internet speed results">
      <div><span>Download</span><strong>{{ speedValue(job.internetSpeed.downloadMbps) }} <small>Mb/s</small></strong></div>
      <div><span>Upload</span><strong>{{ speedValue(job.internetSpeed.uploadMbps) }} <small>Mb/s</small></strong></div>
      <div title="Median TCP connection time to the Cloudflare test server"><span>Latency · TCP</span><strong>{{ speedValue(job.internetSpeed.latencyMs) }} <small>ms</small></strong></div>
      <p class="y-maint__muted">Cloudflare · {{ job.status === 'running' ? 'Testing ' + job.internetSpeed.phase : job.status }} · {{ (job.internetSpeed.transferredBytes / 1000000).toFixed(2) }} MB in completed transfers{{ job.internetSpeed.localIp ? ' · source ' + job.internetSpeed.localIp : '' }}</p>
    </section>
...
export default defineComponent({
  ...
  data() { return {
    tool: 'internet-speed', host: '', device: '', family: 4, count: 4, seconds: 5, mbps: 10, port: 5201, direction: 'upload',
  ...
  methods: {
    speedValue(value: number | null): string { return value === null ? '—' : value.toFixed(1); },
    ...
    async run() {
      ...
      try {
        const job = await maintenanceRequest<DiagnosticJob>('diagnostics', {
          ...
          family: this.family,
          ...(this.tool === 'ping' ? { count: this.count } : {}),
          ...(this.tool === 'bandwidth' ? { seconds: this.seconds, mbps: this.mbps, port: this.port, direction: this.direction } : {}),
        });
...
.y-diag__speed { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); margin-top: 20px; padding: 16px 0; gap: 16px; border-top: 1px solid var(--yonder-divider, #2b333c); }
.y-diag__speed span { color: var(--yonder-label, #7f8a95); font-size: 12px; }
.y-diag__speed strong { display: block; margin-top: 8px; font: 26px var(--yonder-font-mono, monospace); color: var(--yonder-select, #2ad4f0); }
.y-diag__speed small { font: 12px var(--yonder-font, sans-serif); }
.y-diag__speed p { grid-column: 1 / -1; margin: 0; overflow-wrap: anywhere; }
@media(max-width: 450px) { .y-diag__speed { gap: 8px; } .y-diag__speed strong { font-size: 20px; } }
</style>
