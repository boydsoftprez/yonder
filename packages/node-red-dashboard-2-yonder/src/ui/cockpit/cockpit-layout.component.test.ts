...
it('keeps the same PFD and map when the operator enables a stacked MFD',async()=>{
 const w=host(),pfd=w.get('.pfd-svg').element,map=w.findComponent({name:'YonderCockpitMap'}).element;
 await w.get('[aria-label="Display menu"]').trigger('click');await w.get('[aria-label="Display setup"]').trigger('click');await w.get('[aria-label="Display arrangement"]').setValue('stacked');
...
it('saves local layout choices and can hide instruments without hiding navigation fields',async()=>{
 const w=host();await w.get('[aria-label="Display menu"]').trigger('click');await w.get('[aria-label="Display setup"]').trigger('click');await w.get('[aria-label="Instrument placement"]').setValue('hidden');await w.get('[aria-label="Display arrangement"]').setValue('split');expect(w.attributes('data-arrangement')).toBe('split');expect(JSON.parse(localStorage.getItem('yonder-instrument-layout-v1')||'null')?.display.arrangement).toBe('split');w.unmount();
...
it('places the existing bank on the left of the MFD and restores it without replacing the PFD or map',async()=>{
 ...
 await w.get('[aria-label="Display menu"]').trigger('click');await w.get('[aria-label="Display setup"]').trigger('click');await w.get('[aria-label="Display arrangement"]').setValue('stacked');await w.get('[aria-label="Instrument placement"]').setValue('mfd-left');
...
it('reveals the lower MFD when a reading opens its inspector without commanding the aircraft',async()=>{
 const w=host();await w.get('[aria-label="Display menu"]').trigger('click');await w.get('[aria-label="Display setup"]').trigger('click');await w.get('[aria-label="Display arrangement"]').setValue('stacked');
