...
describe('flight workflow boundaries',()=>{
 it('shows actual AUTO while GUIDED has only been requested and acknowledged',()=>{expect(workflow).not.toBeNull();const state=workflow!.flightAnnunciation({...snapshot,operations:[{action:{kind:'mode',customMode:15},state:'accepted',ack:{result:0},effect:{state:'waiting'}}]});expect(state.mode).toBe('AUTO');expect(state.request).toContain('GUIDED');expect(state.request).toContain('accepted');expect(state.outcome).toContain('confirmation');expect(state.outcome).not.toContain('ACK');expect(state.outcome).not.toContain('captured')});
...
it('retains terminal outcomes after an accepted ACK',()=>{
 for(const state of ['unknown','failed','rejected']){const display=workflow!.flightAnnunciation({...snapshot,operations:[{action:{kind:'heading',headingDeg:90},state,ack:{result:0},effect:{state:'unavailable'}}]});expect(display.tone).toBe('caution');expect(display.request).toContain(state==='rejected'?'declined':state);expect(display.request).not.toContain('accepted')}
});
