...
<template>
  ...
  <CockpitOverlay @escape="emit('cancel')">
...
onMounted(() => { nextTick(() => dialog.value?.querySelector<HTMLElement>('select, input, button')?.focus()); });
...
// Slot content is rendered by the overlay host; observe editor state directly
// so disabling/removing the focused control still repairs focus after a portal.
watch([draft,selected,query,()=>props.items],()=>{void restoreEditorFocus()},{deep:true,flush:'post'});
onBeforeUnmount(() => { if(priorFocus?.isConnected)priorFocus.focus(); });
</script>
<style scoped>
