...
function host(){const w=mount(MissionHome,{props:{home:{lat:35,lon:-84,alt:300},terrainEnabled:true,provider:{revision:1}}});hosts.push(w);return w}
it('samples only on request, identifies its prepared source, and saves the estimate only on Save',async()=>{
 mocks.load.mockResolvedValue({waypoints:[{groundM:315.25}],manifest:{title:'Prepared test survey'}});
 const w=host();await flushPromises();expect(mocks.load).not.toHaveBeenCalled();
 await w.findAll('button').find(b=>b.text().startsWith('Use terrain elevation'))!.trigger('click');await flushPromises();
 expect(mocks.load).toHaveBeenCalledWith(expect.objectContaining({datum:'EGM96',route:expect.objectContaining({points:[expect.objectContaining({lat:35,lon:-84})]})}));
 expect(w.text()).toContain('Prepared test survey · EGM96');expect(w.emitted('save')).toBeUndefined();expect(w.emitted('review')).toBeUndefined();
...
it('retains the entered elevation when the terrain source has no compatible sample',async()=>{
 ...
 const w=host();await w.findAll('button').find(b=>b.text().startsWith('Use terrain elevation'))!.trigger('click');await flushPromises();
