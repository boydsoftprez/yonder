<template>
<div class="terrain-vision" :data-terrain-ready="visible" :data-terrain-status="message" :data-terrain-imagery="imageryState" :data-terrain-imagery-tiles="imageryCount" :data-terrain-detail="detailState" :data-terrain-detail-resolution="detailMetresPerPixel" style="position:absolute;inset:0;pointer-events:none;overflow:hidden" aria-hidden="true">
  <canvas ref="canvas" class="terrain-canvas" :style="{visibility:visible?'visible':'hidden'}" style="display:block;width:100%;height:100%"></canvas>
  <div v-if="visible&&(imageryCount||detailState==='ready')" style="position:absolute;bottom:2px;left:4px;right:4px;color:#d7e3ec;text-shadow:0 1px 2px #000;font:9px sans-serif;text-align:right">{{imageryAttribution}}</div>
 </div>
</template>
<script>
...
import {
  loadTerrainPack
} from './terrain-pack-client.mjs';
import {
  viewportProjection
} from './terrain-viewport.mjs';
import {
  ref,
  onMounted,
  onBeforeUnmount
...
const SOURCE = '/cockpit/api/tiles/elevation';
const IMAGERY_SOURCE = '/cockpit/api/tiles/imagery';
const ATTRIBUTION = 'Mapzen terrain · USGS and other elevation providers';
const ATTRIBUTION_URL = 'https://github.com/tilezen/joerd/blob/master/docs/attribution.md';
const IMAGERY_ATTRIBUTION = 'Imagery © Esri, Maxar, Earthstar Geographics, and the GIS User Community';
const IMAGERY_ATTRIBUTION_URL = 'https://www.arcgis.com/home/item.html?id=10df2279f9684e4a9f6a7f08febac2a9';
const CACHE_NAME = 'yonder-terrain-terrarium-v1',
  MAX_CACHED_TILES = 36;
const tileMemory = new Map(),
  imageryMemory = new Map();
// Imagery stays in bounded session memory. Elevations alone use the persistent
// offline cache; a missing photo layer never makes an elevation mesh unusable.
async function imageryTile(x, y, z, signal) {
  const key = `${z}/${x}/${y}`;
  if (imageryMemory.has(key)) {
    const stored = imageryMemory.get(key);
    imageryMemory.delete(key);
    imageryMemory.set(key, stored);
    return stored;
  }
  const work = (async () => {
    const response = await fetch(`${IMAGERY_SOURCE}/${key}`, {
      credentials: 'same-origin',
      signal
    });
    if (!response.ok) throw new Error('Surface imagery unavailable');
    return response.blob();
  })();
  imageryMemory.set(key, work);
  while (imageryMemory.size > 192) imageryMemory.delete(imageryMemory.keys().next().value);
  try {
    return await work;
  } catch (error) {
    if (imageryMemory.get(key) === work) imageryMemory.delete(key);
    throw error;
  }
}
async function imagerySurface(tile, signal) {
  ...
  await Promise.all(Array.from({
    ...
    while (next < side * side) {
      const index = next++,
        ...
        col = index % side;
      const blob = await imageryTile(tile.x * side + col, tile.y * side + row, tile.z + detail, signal),
        bitmap = await createImageBitmap(blob);
      try {
        if (bitmap.width !== 256 || bitmap.height !== 256) throw new Error('Unexpected imagery tile size');
...
async function imageryPatchSurface(patch, signal) {
  const surface = document.createElement('canvas');
  ...
  await Promise.all(Array.from({
    ...
    while (next < patch.tiles.length) {
      const tile = patch.tiles[next++],
        blob = await imageryTile(tile.x, tile.y, tile.z, signal),
        bitmap = await createImageBitmap(blob);
      try {
        if (bitmap.width !== 256 || bitmap.height !== 256) throw new Error('Unexpected imagery tile size');
...
async function elevationTile(x, y, z, signal) {
  const key = `${z}/${x}/${y}`;
  ...
  const work = (async () => {
    let cache = null,
      response = null;
    // Device service owns any persistent terrain cache.
    if (!response) {
      response = await fetch(url, {
        credentials: 'same-origin',
        signal
      });
      if (!response.ok) throw new Error('Elevation tiles unavailable');
      if (cache) try {
        await cache.put(url, response.clone());
        const keys = await cache.keys();
        await Promise.all(keys.slice(0, Math.max(0, keys.length - MAX_CACHED_TILES)).map(k => cache.delete(
          k)));
      } catch {
        /* Quota failure must not prevent live terrain. */ }
    }
    ...
    try {
      if (bitmap.width !== 256 || bitmap.height !== 256) throw new Error('Unexpected terrain tile size');
      const surface = document.createElement('canvas');
  ...
  while (tileMemory.size > MAX_CACHED_TILES) tileMemory.delete(tileMemory.keys().next().value);
  try {
    return await work;
  } catch (error) {
...
function createRenderer(canvas) {
  ...
  const terrainProgram = program(gl, `
 ...
 `);
  ...
  const uploadTexture = surface => {
    ...
    if (anisotropy) gl.texParameterf(gl.TEXTURE_2D, anisotropy.TEXTURE_MAX_ANISOTROPY_EXT, Math.min(8, gl
      .getParameter(anisotropy.MAX_TEXTURE_MAX_ANISOTROPY_EXT)));
    return texture;
  };
  const disposeMeshes = () => {
  ...
  return {
    ...
    replace(data) {
      ...
      disposeMeshes();
      clearDetail();
      if (data.some(m => m.indices instanceof Uint32Array) && !index32) throw new Error(
        '32-bit terrain indices unavailable');
    ...
    draw(pose, origin) {
      ...
      const grid = origin.utm ? latLonToUtm(pose.lat, pose.lon, origin.zone) : null,
        north = origin.utm ? latLonToUtm(pose.lat + .00001, pose.lon, origin.zone) : null,
        convergence = grid ? Math.atan2(north.eastingM - grid.eastingM, north.northingM - grid.northingM) * 180 / Math
        .PI : 0;
      const basis = cameraBasis(pose.heading + convergence, pose.pitch, pose.roll),
        viewport = viewportProjection(width, height);
      ...
      const position = grid ? [grid.eastingM - origin.eastingM, 0, origin.northingM - grid.northingM] : localPosition(
        pose.lat, pose.lon, origin);
      ...
      for (const mesh of meshes) {
        ...
        gl.uniform4fv(terrain.detailTransform, detailPatch ? (mesh.packImagery ? [-detailPatch.x / detailPatch.side, -
          detailPatch.y / detailPatch.side, 1 / detailPatch.side, 1 / detailPatch.side
        ] : terrainImageryTransform(mesh, detailPatch)) : [0, 0, 0, 0]);
        ...
        for (const chunk of mesh.chunks)
          if (terrainChunkVisible(chunk, frustum)) {
            const dx = Math.max(chunk.minimum[0] - position[0], 0, position[0] - chunk.maximum[0]),
              dz = Math.max(chunk.minimum[2] - position[2], 0, position[2] - chunk.maximum[2]);
            const coarse = chunk.coarseCount && Math.hypot(dx, dz) > 1500;
            gl.drawElements(gl.TRIANGLES, coarse ? chunk.coarseCount : chunk.count, mesh.indexType, (coarse ? chunk
              .coarseOffset : chunk.offset) * mesh.indexBytes);
          }
      }
...
export default {
  props: ['flight', 'telemetry', 'enabled', 'displayPose', 'imageryEnabled', 'lookaheadSeconds'],
  emits: ['status'],
  setup(props, {
    emit
  }) {
    ...
    const status = (state, text) => {
      ...
      const key = [JSON.stringify(forecast), state, text, imageryState.value, imageryCount.value, detailState.value,
        Math.round((ground?.estimatedAglM ?? 0) / .3048), Math.round(ground?.groundElevationM ?? 0)
      ].join('|');
      ...
      emit('status', {
        ...
        attribution: pack ? pack.manifest.sources.map(s => s.attribution).join(' · ') : ATTRIBUTION,
    ...
    const loadDetail = async (pose) => {
      ...
      try {
        const image = await imageryPatchSurface(patch, active.signal);
      ...
      try {
        ...
        await Promise.all(Array.from({
          ...
          while (next < tiles.length) {
            ...
            try {
              const image = await imagerySurface(tile, currentController.signal);
        ...
        if (disposed || token !== generation) return;
        imageryState.value = failed ? (imageryCount.value ? 'partial' : 'unavailable') : 'ready';
      } finally {
        clearTimeout(timeout);
      }
    };
    const load = async (pose, key, center) => {
      ...
      status(origin ? 'ready' : 'loading', origin ? 'Updating terrain region…' : 'Loading elevation terrain…');
      try {
        let candidate = null;
        try {
          candidate = await loadTerrainPack(pose, props.telemetry?.altitudeDatum || 'UNKNOWN', currentController
            .signal)
        } catch {
        ...
        for (let row = -1; row <= 1; row++)
          for (let col = -1; col <= 1; col++) requests.push({
            x: (center.x + col + 2 ** TERRAIN_ZOOM) % 2 ** TERRAIN_ZOOM,
            y: center.y + row
          });
        if (requests.some(t => t.y < 0 || t.y >= 2 ** TERRAIN_ZOOM)) throw new Error(
          'Terrain coverage unavailable at this latitude');
        const tiles = [];
        let next = 0;
        await Promise.all(Array.from({
          length: 3
          ...
          while (next < requests.length) {
            ...
            tiles.push(await elevationTile(tile.x, tile.y, TERRAIN_ZOOM, currentController.signal));
          }
        }));
        if (disposed || token !== generation) return;
        const newOrigin = tileCoordinate(center.x + .5, center.y + .5, TERRAIN_ZOOM),
        ...
        loadingKey = '';
        failedUntil = 0;
        tiles.sort((a, b) => (Math.abs(a.x - center.x) + Math.abs(a.y - center.y)) - (Math.abs(b.x - center.x) +
          Math.abs(b.y - center.y)));
        ...
        } else status('unavailable', navigator.onLine ? 'Elevation unavailable · conventional horizon' :
    ...
    const animate = time => {
      ...
      const tile = tilePosition(pose.lat, pose.lon, TERRAIN_ZOOM),
        ...
        key =
        `${center.x}/${center.y}/${Math.floor(pose.lat*1000)}/${Math.floor(pose.lon*1000)}/${props.telemetry?.altitudeDatum}`;
      if (regionKey !== key) {
        if (!loadingKey && time >= failedUntil) void load(pose, key, center);
        else if (loadingKey) status(origin ? 'ready' : 'loading', origin ? 'Updating terrain region…' :
          'Loading elevation terrain…');
        if(!origin)return;
      }
      if(pack&&pack.manifest.verticalDatum!==props.telemetry?.altitudeDatum){clearance.value=null;status('unavailable','Height reference changed · awaiting compatible terrain');return;}
      if(!Number.isFinite(props.flight.vsi)||!Number.isFinite(props.flight.groundspeed)||!Number.isFinite(props.telemetry.trackDeg))forecast=null;
      try {
        clearance.value = pack ? terrainClearance(pose, groundSampler) : null;
        if (time - lastForecast >= 500) {
          forecast = pack ? evaluateTerrainPath({
            lat: pose.lat,
            lon: pose.lon,
            ...
            telemetryFresh: props.flight.live && Number.isFinite(props.flight.groundspeed) && Number.isFinite(
              props.flight.vsi),
            transformVerified: pack.manifest.verticalTransform.verified,
            sample: pack.sampleBoth,
            lookaheadSeconds: Math.max(1, Math.min(120, props.lookaheadSeconds || 30)),
            stepSeconds: 1,
        ...
        if (props.imageryEnabled && detailKey !== terrainImageryPatch(pose).key && !detailLoadingKey && time >=
          detailFailedUntil) void loadDetail(pose);
        if (time - lastDraw >= 50) {
          renderer.draw(props.displayPose || pose, origin);
          lastDraw = time;
        }
        status('ready', pack ?
          `Cove terrain · ${pack.spacingM} m nearby / ${pack.farSpacingM} m distant · mapped surface · ${pack.manifest.verticalDatum}` :
          'Terrarium elevation · assumed MSL · ' + (detailState.value === 'ready' ? 'nearby surface detail' : ({
