...
const legData=computed(()=>props.mission.items.map((p,i)=>{const a=props.mission.items[i-1];return validPosition(a)&&validPosition(p)&&[16,22,84].includes(a.command)&&p.command===16?{course:Math.round(bearing(a,p))+'° T',length:(distance(a,p)/1852).toFixed(2)}:null}));
