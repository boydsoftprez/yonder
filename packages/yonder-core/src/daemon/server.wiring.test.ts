  ...
  it("takes one frame per watched camera on its own clock, and serves it with its age", async () => {
    ...
    try {
      ...
      const page = (await call(socketPath, "GET", "/cameras/cam0")).body as {
        strip: { cameras: { id: string; active: boolean; thumbSrc: string | null; stopped: boolean }[]; downlink: string };
      };
      expect(page.strip.cameras.map((c) => [c.id, c.active, c.thumbSrc, c.stopped])).toEqual([
        ["cam0", true, null, false],
        ["cam1", false, null, false],
      ]);
      expect(page.strip.downlink).toMatch(/^\d+ kb\/s of stills · counted in Path total$/);
      expect(page.strip.downlink).not.toMatch(/^0 kb/);
      ...
      const later = (await call(socketPath, "GET", "/cameras/cam0")).body as {
        strip: { cameras: { id: string; thumbSrc: string | null; ageSeconds: number | null }[] };
      };
      expect(later.strip.cameras[1]).toMatchObject({ id: "cam1", ageSeconds: 0 });
      expect(later.strip.cameras[1]?.thumbSrc).toMatch(/^\/video\/cam1\/still\?at=\d+$/);
    } finally {
      await server.close();
    }
  });
...
describe("buildRenderers", () => {
  ...
  it("puts the telemetry renderer last, when it is given a way to open a serial port", () => {
    ...
    // `video` joins the list on merge: the camera pipelines are pushed last
