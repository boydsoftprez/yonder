...
export class Stills {
  ...
  private readonly held = new Map<string, LatestStill>();
  ...
  latest(camera: string): LatestStill | null {
    if (!this.alive(camera)) return null;
    return this.held.get(camera) ?? null;
  }
  ...
  read(camera: string): Answer<StillBody> {
    ...
    try {
      ...
      return {
        ok: {
          ...held, bytes: body.length, body, contentType: "image/jpeg",
   ...
   *
   * `at` is stamped when the frame is asked for, not when the host answers:
   * the frame is the first buffer after the branch joins the tee, which is
   * within a frame period of the request, and the host's reply comes a
   * continuity watch later. The age a viewer reads is the frame's, not the
   * reply's.
   */
