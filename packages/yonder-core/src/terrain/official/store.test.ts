...
async function staged(store: OfficialTerrainStore, height = 123): Promise<TileRecord & {path: string}> {
  ...
  return {path, tile: 'N35W084', sha256: hash.digest('hex'), archiveSha256: 'a'.repeat(64), bytes: HGT_TILE_BYTES as 25934402,
    sourceUrl: 'https://terrain.ardupilot.org/SRTM1/N35W084.hgt.zip', retrievedAt: new Date().toISOString()};
}
...
describe('OfficialTerrainStore', () => {
  ...
  it('removes interrupted staging on restart and refuses unknown manifest versions', async () => {
    const {store, root, options} = await setup();
    ...
    await reopened.close(); await writeFile(join(root, 'index.json'), '{"schemaVersion":2,"objects":[],"areas":[]}');
