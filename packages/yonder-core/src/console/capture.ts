...
export function captureHandler(socketPath: string, timeoutMs = CAPTURE_TIMEOUT_MS): CaptureHandler {
  return (req) => new Promise<CaptureAnswer>((resolve) => {
    if (!CAMERA_ID.test(req.camera) || !CAPTURE_NAME.test(req.name)) {
      resolve(refused(404, "that is not a capture on this device"));
      return;
    }
    let settled = false;
    ...
    const outgoing = request(
      {
        socketPath,
        method: "GET",
        path: `/cameras/${req.camera}/captures/${encodeURIComponent(req.name)}`,
      },
