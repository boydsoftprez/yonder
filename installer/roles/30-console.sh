...
# node-red-dashboard-2-yonder is here for the same reasons and one more: its
# dist/ carries the built Vue bundles as well as the nodes, and Dashboard
...
for pkg in node-red-contrib-yonder-system node-red-contrib-yonder-network \
    ...
    # The widget package also ships built Vue bundles. Node-RED serves a
    # package's static assets from `<module>/resources`, which is the URL
    # Dashboard asks for; without them every widget instantiates and the page
    # renders an empty box while the browser collects 404s.
    if [ -d "$pkg_src/resources" ]; then
        run cp -r "$pkg_src/resources" "$pkg_dest/resources"
    fi
done
...
# express.static.
#
# Dashboard finds a third-party widget package by reading the **user
# directory's** package.json for a dependency whose name contains
# `node-red-dashboard-2-`, and then resolving it at
# `<userDir>/node_modules/<name>` (its `getThirdPartyWidgets`). A package that
# is merely present in the console tree's node_modules - which is where
# Node-RED finds its *nodes* - is invisible to that scan.
#
# The two halves are separate and both are needed. Without the node, the flow
...
if [ "$DRY_RUN" = "1" ]; then
    ...
    ensure_dir "$con_state/node_modules" 0755
    rm -f "$con_state/node_modules/node-red-dashboard-2-yonder"
    ln -s "$con_dest/node_modules/node-red-dashboard-2-yonder" \
          "$con_state/node_modules/node-red-dashboard-2-yonder"
    con_widget_version=$(node -p 'require(process.argv[1]).version' "$con_dest/node_modules/node-red-dashboard-2-yonder/package.json")
    cat > "$con_state/package.json" <<MANIFEST
{
  "name": "yonder-console-state",
  "version": "0.0.0",
  "private": true,
  "_comment": "GENERATED by installer/roles/30-console.sh. Node-RED's user directory is not an npm project; this file exists only so Dashboard 2.x discovers Yonder's widget package, which it does by reading dependencies here. See getThirdPartyWidgets in @flowfuse/node-red-dashboard.",
  "dependencies": {
    "node-red-dashboard-2-yonder": "$con_widget_version"
  }
}
MANIFEST
    chown yonder:yonder "$con_state/package.json"
    chown -h yonder:yonder "$con_state/node_modules/node-red-dashboard-2-yonder" 2>/dev/null || true
fi
