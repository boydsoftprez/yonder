...
### K-06 · ~~No request body size limit~~ — CLOSED
`src/daemon/server.ts`

Request bodies accumulated unbounded; 5 MB went through without complaint. Behind a
local Unix socket restricted to the service group, that was still enough for a mistaken request to kill the configuration
daemon on a 512 MB board.

Closed under R-SEC-15. The daemon retains at most 1 MiB for a route without a narrower
contract; existing 8 KiB owner/recovery/storage and 512 KiB cockpit limits remain stricter,
while recovery preview retains its larger authenticated archive allowance. A declared or
streamed oversize request is discarded through its last byte before the daemon sends 413, so
the caller receives the reason rather than a broken pipe and the daemon remains available.
`server.test.ts` covers declared and streamed default-cap refusals, an exactly 1 MiB request
reaching its route, and coexistence with the cockpit and recovery-preview limits.
...
### K-69 · ~~The cockpit guide cannot run to the end~~ — CLOSED

**Requirements:** R-FLT-10, R-UI-12, R-FLT-25 · **Found and closed:** 2026-09-12

The fixture guide still pressed the removed `PFD Menu` and `Display & data`
controls, so it stopped before later mission/data and camera checks. Its four camera
captures had therefore been produced by a separate temporary fixture harness.

The restored guide and sibling scripts now use the app-header **Display** menu,
current palette label and HOME marker. The terrain walkthrough also follows main's
separate official-ground and detailed-display contracts. The fixture guide passes
all 11 workflow groups and 28 captures; terrain streaming, datum refusal, complete
preload and offline reuse pass with no aircraft requests. The four camera images
are now regenerated by the guide and their manifest provenance is `guide`.

See [the consolidation record](console/evidence/2026-09-12-fix-consolidation.md).
The simulator walkthrough and profiling scripts received the same selector repair
by inspection; no simulator flight or hardware acceptance is claimed.

### K-70 · The daemon starves its own telemetry reader on the Radxa bench board — OPEN, software half closed

**Requirements:** R-HW-07, R-CAM-24, R-MAV-10, R-FLT-02 · **Found:** 2026-09-13 · **Owner of what remains:** JJ Boyd

Seen on the Radxa ZERO 3 bench board running the 2026.9.0 tree with the SeekerHD
streaming and the Flight page open: attitude, mode and arm state came and went, the
camera window vanished and returned, and the flight display read *No data · 0.0 Hz*
while the router was reading the flight controller on UART2 without trouble. The
kernel counter on the daemon's telemetry socket (`127.0.0.1:14559`) showed its
receive queue full at about 180 KB and **117 datagrams a second being dropped**. The
daemon's main thread was 54 % busy and the board's load average was 6 on four cores.

A 30 s CPU profile of the running daemon (V8 sampling profiler over the inspector,
no restart) said where the thread went:

| Share of the main thread | What |
|---|---|
| 51 % | `spawn` — forking helper processes; about ten a second |
| 20 % | camera detection (`detectCameras` → `probeNode` → `v4l2-ctl`, 21 per sweep on ten ISP nodes, a sweep every 5 s from three callers) |
| 9 % + 8 % | `mmcli` for the Cellular page and the instruments, `zerotier-cli` for the mesh state, each polled fresh every 2 s and 5 s |
| 16 % | the DJI Pocket 2 accessory stream: base64 JSON lines from the USB helper at 1.35 MB/s, decoded and parsed with no camera consuming them |
| 3 % | MAVLink decoding — the work the thread exists to do |

The journal for five minutes counted 250 `mmcli -L`, 245 `mmcli -m`, 208 of each of
three `zerotier-cli` verbs, 132 + 129 `nmcli`, 129 `ip route`, 113 `mmcli -b` and
112 `mmcli --signal-get`. The USB helper itself held a third of a core, and the
camera pipeline two thirds, encoding a 1920×1080 preview at 30 fps because the
camera's own configuration asked for that rung (an operator selection under
R-VID-13, not a defect; 640×360 or 1280×720 at 15 fps is what the flight display
can show).

**Closed by this change:** camera probes are reused between sweeps and shared while
in flight (R-CAM-24), and the ISP's nine auxiliary nodes are remembered as such;
the modem and mesh readings the console polls are memoised for 3 s and 5 s on
clients of their own, leaving the modem port and the reach monitor fresh
(R-CEL-13); ZeroTier's reads run concurrently; a missing `vcgencmd` is asked once.
The wiring test proves a second `/modem/state` within the window forks nothing.

**Measured on the same board, same page, same camera, an hour later** (the daemon
replaced in place and profiled again for 30 s): forking fell from 51 % to 18 % of
the main thread, idle time rose from 3.5 % to 17 %, `v4l2-ctl` and `zerotier-cli`
no longer appear among the processes being born, the telemetry socket's queue drains
to zero with **no drops**, the heartbeat reads 1 Hz and the last frame was heard
0.4 s ago rather than 4 s. MAVLink decoding is now 10 % of the thread, which is the
work it is for.
