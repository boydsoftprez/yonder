...
| R-FLT-25 | Retain a single PFD with expandable insets as the default, and offer separate adjacent or stacked multifunction panes for map, flight plan/profile and systems. Allow the instrument bank beside the PFD, across the top, on the MFD or hidden. Offer operator-invoked full screen with a visible exit where the browser supports it. Keep cockpit palette selection independent of the surrounding console, with readable instrument contrast in both palettes. Preserve the flight display and reviewed command context during layout and full-screen changes; saved settings remain validated and usable at laptop and tablet sizes | 1 |
