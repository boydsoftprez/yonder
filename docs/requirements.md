...
| R-CEL-12 | **What Yonder reports about a modem is read from the modem that is there now, never remembered from one that was.** A modem's port layout may be held on to while that modem is present, because it is a property of the modem; *that a modem exists* may not, because it is a property of the moment. A board whose modem is unplugged, powered off or claimed by nothing reports no cellular path at all — it does not go on naming the interface the departed modem carried traffic on, and it neither tests nor stands down a path that is not there. A reading that could not be taken is not an answer and changes nothing: only the modem service *saying* it has no modem clears what was remembered | 1 |
