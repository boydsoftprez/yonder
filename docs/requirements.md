...
| R-VID-16 | **Start and stop each output independently from the console, and show what each is costing while it runs** | 2 |
| R-VID-17 | **Adapt the preview's size as well as its bitrate**, down a ladder when pinned at the floor and up when there is headroom, within an operator-set top and bottom; a chosen size holds | 2 |
