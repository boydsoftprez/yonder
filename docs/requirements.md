...
| R-CAM-15 | Capture from a camera that is itself a USB host and expects a phone, by presenting the board as that phone and speaking the camera's own protocol. The picture and the gimbal, exposure and white-balance controls (R-CAM-11) arrive on the same link. Video reads and queued video must not block command dispatch or completion reporting; original command deadlines remain authoritative at kernel submission and across partial transfers. Requires a board port that can act as a USB device, which on a Raspberry Pi 4 means header or PoE power | 3 |
...
| R-CAM-20 | **Every configured camera is on the camera list, present or not.** A camera the device is configured for and cannot find is stated there as configured and not attached, carrying the socket it expects — never left off the list, and never drawn as merely idle. The two are different facts and an operator has to act on them differently: an idle camera is one they can start, and an absent one is a plug to move or an entry to remove. Leaving it off is worse than either, because the list is the answer to *what cameras does this device have* and the rest of the interface is still built from the same configuration — so the list and the navigation disagree, in the place an operator goes to find out which is right | 1 |
| R-CAM-21 | **A camera can be taken out of the configuration from the interface.** A camera's identity is the socket it is attached to (R-CAM-05), so moving it to another port makes it a different camera and leaves the previous entry behind expecting a device that is not there; nothing else ever removes it. An entry the interface cannot remove is one an operator has to edit the configuration file by hand to be rid of, which is the thing R-CFG-01's single writer exists so that nobody has to do. The removal goes through the confirmation timer like every other configuration change (R-CFG-03), **is refused while that camera is streaming** — with the reason stated, because stopping a stream is its own control and removing the camera is not a way to press it — and leaves the captures the camera has already made on the device (R-CAM-18) | 1 |
| R-CAM-22 | **Restore configured camera streams at boot without operator action.** Each enabled camera with `autostart: true` starts from its saved device identity and stream settings. Discovery retries when the camera or encoder is not ready, and automatic pipelines retry with bounded backoff while their media server is unavailable. Startup work never blocks the console or network. Start and Stop take precedence for the current daemon lifetime; a later apply must not resurrect an operator-stopped camera. Configuration cancellation remains reversible, so rollback restores a camera it suspended | 1 |
...
| R-VID-07 | Adapt encoder bitrate to receiver-observed link conditions, within an operator-set floor and ceiling. Retain loss, delay and delivery feedback when the browser lacks a capacity estimate; use bounded probes, label their evidence, recover after congestion, and show actual encoder readback. Never treat silence as spare capacity or a local receiver as proof of an independent LTE path. Include external RTSP receivers: use their reported packet loss and server discards, plus acknowledged delivery and queue pressure for TCP. Reduce promptly under congestion, recover gradually within the applied bounds, and keep Fixed mode unchanged. Show whether feedback is active, waiting, incomplete or unavailable; state when congestion remains at the minimum instead of claiming the stream fits | 2 |
| R-VID-08 | Allow a fixed bitrate where the operator prefers determinism. Verify actual encoded output as well as the configured target | 1 |
...
| R-NET-17 | Network and Status display current kernel addresses for every Ethernet, Wi-Fi, cellular and ZeroTier interface, including IPv4 and IPv6, carrier, gateways and route metrics. Failed or stale observations clear old addresses. Default-route status is observed from the kernel, separate from configured priority and ZeroTier peer path selection. Applied route metrics take effect on active owned profiles without cycling the link, including on rollback. | 1 |
...
| R-DIA-07 | Run bounded, cancellable ping, traceroute and route lookup tests against operator-selected hostnames or IPv4/IPv6 addresses and interfaces. Show actual incremental stdout/stderr as plain terminal text, completion/exit status and downloadable output. Jobs belong to the initiating authenticated session, use fixed command arguments without a shell, and expire. Advanced bandwidth tests use an explicitly selected iperf3 server with visible direction, duration and rate ceiling. | 2 |
| R-DIA-08 | Diagnostics offers an explicitly confirmed system reboot with a stated delay and interruption of console, video and telemetry. Refuse it while configuration is pending or the aircraft is reported armed. | 2 |
| R-DIA-09 | Offer a one-action public internet speed test from Yonder itself, with no operator-managed server. Show download/upload estimates, TCP latency, provider, source address, progress and a stated payload/time limit. Allow interface and IP-family selection, cancellation, and output download. Use synthetic payloads and fixed HTTPS test endpoints, run only on explicit operator action, and retain iperf3 as an advanced option. | 2 |
...
| R-SEC-14 | Settings allows an authenticated operator to change the administrator password by entering the current password and confirming the new one. Apply existing password rules, throttle failed current-password attempts, persist durably, redact every submitted secret, and invalidate existing console and flow-editor sessions. Password fields and requests must not enter flow context or dashboard message history. | 1 |
...
| R-UI-07 | Provide **day and night themes as equal, operator-selectable modes**, defaulting to day. The choice persists across reboots and is never overridden by the host or the browser. Day/night-only requests render appearance without invoking network, modem or camera configuration, while retaining transactional persistence and rollback | 1 |
...
| R-UI-10 | **Size a control to what it says, and put it beside the thing it acts on.** No action occupies the full width of the surface it sits on, except below a viewport too narrow for anything else. A page has at most one primary action. **An action whose subject is one control or one panel is drawn beside that control or panel** — Record under the picture, Recentre on the aim panel, an output's on/off in the row that states its cost — because an operator watching a picture must not have to look away from it to press the key that acts on it. **The soft-key rail carries the page's own actions**, the ones whose subject is the page rather than anything on it, and it is where an irreversible action goes ([ADR-0009](adr/0009-console-visual-language.md)) | 2 |
...
| R-UI-21 | **A control another setting has charge of stays on the page, drawn inert, naming the setting that has it** in the operator's words. It is not a fault and is not drawn as one | 2 |
| R-UI-22 | **The first paint carries the theme.** The generated stylesheet is linked from the served document's head, fetched with the document itself, so the browser never paints an unstyled page before it arrives. On a night flight that flash is a torch in the operator's face, on every load, not only the first. No stylesheet reaches the console through a script or a widget after the page has already rendered | 1 |
| R-UI-23 | **The capture gate photographs readings, not masks.** Every field renders its widest honest specimen, and the gate measures clipped text | 2 |
| R-UI-24 | **An output nothing can reach is drawn as unreachable, and its stream address marked unusable rather than offered.** The console states which of the device's paths can carry it, and does not act | 1 |
| R-UI-25 | **The instrument library is rendered whole, from source, in both palettes, in every state, on every build** | 2 |
...
| R-UI-29 | **The camera workspace has one picture, one aim panel, and one persistent place to apply and confirm changes.** The live picture is the primary element: it fills its column, preserving its aspect ratio, with compact supporting controls beneath it. Camera thumbnails carry real preview frames and their age. Pan/tilt readings sit with their controls outside the picture. Camera image controls, capture controls and stream configuration remain available on one surface; variable messages and controls fit their containers, and routine updates and operation results appear inline without covering other content. Put video readings in a solid strip above the image, show unsaved values and highlights on their controls, and always expose the RTSP enable control Start, Stop, Retry and sign-in actions reflect the observed lifecycle. Local preview modes never stop the camera. Connection details expand within the workspace on demand, showing stable camera identity and copyable receiver settings with reachability explanations. Unsaved drafts survive reload and sign-in without replaying commands; Apply, Keep and Revert show their progress and completion on this page | 2 |
| R-UI-30 | Provide a Settings page for administrator password changes and the saved day/night palette. Status repeats the ZeroTier interface identity, byte counts, throughput and traffic history instruments from Network. | 2 |
...
| R-FLT-02 | Authenticate flight reads and writes through the console. Attach server-derived session provenance and a securely generated operation ID to each explicit operator request, including on the supported HTTP hardware console; require review and confirmation for commands that change aircraft behavior. Opening a page, polling or reconnecting never issues a flight command | 1 |
...
| R-FLT-04 | Serialize mission transfers and command transactions. Validate target vehicle generation and mission revision; distinguish sent, acknowledged, observed, rejected and unknown outcomes. Preserve mission item parameters, sequence references, frames and home semantics during import/edit/export. Provide an editable planning home with map selection, MSL elevation, unit conversion and Undo, without changing waypoint values or controller home. Show reported controller home separately; copying it into the plan is local. Setting controller home requires explicit review of old/new references, rejection of changed home context, and matching readback after acceptance. Offer aircraft mission read beside upload while retaining the local draft; distinguish missing readback from completed readback without a controller home record | 1 |
...
| R-FLT-11 | Source public traffic, imagery and display terrain on the ground browser by default, with bounded local history/cache and explicit offline preparation. Distinguish an empty traffic search from missing aircraft position, provider refusal and rate limiting; honor provider cooldowns, including across clients using the same ground relay. Require fresh coordinates and a reported GPS fix before centering ownship or requesting nearby traffic; show the missing-fix condition and automatically resume when valid. Permit a ground traffic relay independently of imagery and terrain sourcing. Aircraft-proxied public downloads require explicit selection and never become an automatic fallback | 1 |
| R-FLT-12 | Carry compact fresh flight updates independently of mission items, command history and geographic data. Default browser flight reads to an operator-selectable, persisted 8 Hz target, accounting for request time without overlapping reads. Show achieved browser and distinct attitude observation rates separately from the requested controller rate. Smooth valid attitude between received samples even without GPS, while withholding geographic poses from terrain and traffic. Transfer stable details on change or explicit request; expose source and link usage so aircraft bandwidth can be budgeted | 1 |
...
| R-FLT-25 | Retain a single PFD with expandable insets as the default, and offer separate adjacent or stacked multifunction panes for map, flight plan/profile and systems. Allow the instrument bank beside the PFD, across the top, on the MFD or hidden. Offer operator-invoked full screen with a visible exit where the browser supports it. Use the app header for a single responsive cockpit control row, preserving the outer brand and keeping RTL visible while overflow actions share the same editors. Dialogs must clear the header and remain usable in fullscreen. Keep cockpit palette selection independent of the surrounding console, with readable instrument contrast in both palettes. Preserve the flight display and reviewed command context during layout and full-screen changes; saved settings remain validated and usable at laptop and tablet sizes | 1 |
