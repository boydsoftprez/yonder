...
The four `camera-*.png` images were regenerated by the complete fixture guide
on 2026-09-12 after restoring its header Display-menu routes (K-69).
Their manifest rows now say `source: "guide"`. The guide reaches all eleven
workflow groups, including the camera check, with aircraft HTTP blocked.
...
Thirteen guide/terrain images were regenerated and reviewed during the September 12
consolidation. The terrain profile now shows official ground and AGL unavailable
beside the separately sourced Cove display surface. `mission-loiter.png`,
`slip-turn.png` and `wind.png` retain their earlier captures because the rerun only
changed 52–68 antialiasing pixels. The `cockpit-*.png` layout set is unchanged.

## Regenerate and verify

From the repository root, build `yonder-core` (the fixture resolves its
`cockpit-wire` and `terrain` exports from `dist/`), then start the fixture:
