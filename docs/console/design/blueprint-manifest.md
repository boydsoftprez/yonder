...
| C-18 | The uplink figure `3.4 of 3.2 Mb/s`, in caution when over | `4.5 of 5.0 Mb/s` over parts of 2.1, 2.1 and 0.4 | drifted | **An earlier reading of this row said the console was wrong here, and it is not.** It reported `1356.0 of 5.0 Mb/s` against a breakdown summing to 4.6, and called the disagreement a defect. `1356.0` is the *specimen* — `nrdb-ui-yonder-budget`'s widest honest value (R-UI-23), eight cameras at the schema's ceiling — and it appears only in `docs/console/capture/`, the copy the gate writes with specimens substituted. `vendor/capture/cameras.night.png`, the page as it really was, reads `4.5 of 5.0 Mb/s`. **The lesson is about the gate, not the console:** a specimen on a total whose parts are drawn separately makes that capture disagree with itself by construction, and an audit read from the specimen copy will find a defect that is not there. One real thing remains, and it is small: each part is rounded on its own (2.07 → 2.1) while the total rounds 4.54 → 4.5, so the drawn parts sum to more than the drawn total |
