...
| F-31 | The selected camera's configured preview fills the attitude scene edge to edge with the screen-fixed instruments over it; none of the picture's own toolbar, badges, notices or strip appears inside the PFD; no painted split behind the picture; the attitude line stays available over the picture with a switch in PFD settings | Specified in [the camera in the flight display](../../superpowers/specs/2026-09-11-flight-camera-in-pfd-design.md); R-FLT-29, R-FLT-09, R-VID-13. Render `instrument-library/flight.camera.full.night.png`. Not yet built; K-68 |
| F-32 | A camera window, home in the top left of the scene, movable by its header and resizable from its corner, place and size remembered locally and reset from PFD settings; synthetic terrain fills the scene behind it; its header carries the stale age and a maximize control | Same spec; R-FLT-29, R-FLT-25. Render `instrument-library/flight.camera.window.night.png` (composite). Not yet built |
| F-33 | One Camera button in the top row beside full screen, with a camera-and-arrows glyph, the same height as its neighbours, reading the current state (full or window) and flipping it; disabled and reading unavailable when no camera is selected; never starts or stops a stream | Same spec; R-FLT-29, R-CMD-04. Render `instrument-library/flight.camera.toggle.night.png` and its zoom. Not yet built |
| F-34 | Height above ground and the forward-clearance forecast keep reporting while the camera fills the scene; a stale picture keeps its hatch and desaturation in the scene and shows its age in the footer label or the window header | Same spec; R-FLT-29, R-FLT-08, R-VID-03. Not yet built; evaluation must not depend on the terrain renderer drawing |
