...
| F-31 | The selected camera's configured preview fills the attitude scene edge to edge with the screen-fixed instruments over it; none of the picture's own toolbar, badges, notices or strip appears inside the PFD; no painted split behind the picture; the attitude line stays available over the picture with a switch in PFD settings | Specified in [the camera in the flight display](../../superpowers/specs/2026-09-11-flight-camera-in-pfd-design.md); `YonderPicture.vue` (`scene` prop), `YonderCockpit.vue`, `PrimaryFlightDisplay.vue`, `cockpit.css`; R-FLT-29, R-FLT-09, R-VID-13, `camera-scene.component.test.ts`. Render `instrument-library/flight.camera.full.night.png`; captured live at [`camera-full.png`](../../images/cockpit/camera-full.png). K-68 closed; hardware line pending |
| F-32 | A camera window, home in the top left of the scene, movable by its header and resizable from its corner, place and size remembered locally and reset from PFD settings; synthetic terrain fills the scene behind it; its header carries the stale age and a maximize control | Same spec; `CameraWindow.vue`, `camera-view.mjs`, `pfd-controls.mjs`, `PfdControlPanel.vue`, `YonderCockpit.vue`; R-FLT-29, R-FLT-25, `camera-view.test.ts`, `camera-window.component.test.ts`. Render `instrument-library/flight.camera.window.night.png` (composite); captured live at [`camera-window.png`](../../images/cockpit/camera-window.png) |
| F-33 | One Camera button in the top row beside full screen, with a camera-and-arrows glyph, the same height as its neighbours, reading the current state (full or window) and flipping it; disabled and reading unavailable when no camera is selected; never starts or stops a stream | Same spec; `YonderCockpit.vue`, `cockpit.css`; R-FLT-29, R-CMD-04, `camera-window.component.test.ts`, guide-measured height. Render `instrument-library/flight.camera.toggle.night.png` and its zoom |
