...
| F-16 | The notice and its switch appear only when no camera is configured; a configured camera that is not streaming states its reason on the picture (decision 2 as amended) and the Camera control (F-33) is the way back to terrain | `YonderCockpit.vue`; R-FLT-09, camera-fallback component test. Terrain coverage/datum and registration limits remain visible. Operator ratified 2026-09-12 after the whole-branch review |
