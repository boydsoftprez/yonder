...
| F-32 | A camera window, home in the top left of the scene, movable by its header and resizable from its corner, place and size remembered locally and reset from PFD settings; synthetic terrain fills the scene behind it; its header carries the stale age and a maximize control, which works because the header's drag gesture yields to controls inside it; the window carries the picture's short stopped message and the resume control, and a longer reason is read at full size (open question on decision 2's amendment) | Same spec; `CameraWindow.vue`, `camera-view.mjs`, `pfd-controls.mjs`, `PfdControlPanel.vue`, `YonderCockpit.vue`; R-FLT-29, R-FLT-25, `camera-view.test.ts`, `camera-window.component.test.ts`. Render `instrument-library/flight.camera.window.night.png` (composite); captured live at [`camera-window.png`](../../images/cockpit/camera-window.png) |
