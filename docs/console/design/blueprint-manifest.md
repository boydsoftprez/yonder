...
120°/s, defaulting to 30°/s. Expo remains shared with pan/tilt; Shift supplies fine
...
When collapsed, Aim leaves a compact 56 × 44 px **‹ Aim** tab at the preview's
right edge (R-UI-29). The surrounding Dashboard card has no background, border,
shadow or padding in this state. Reopening preserves the video receiver and the
camera's saved drawer preference; expanded layout behavior is unchanged.
...
| F-16 | The notice and its switch appear only when no camera is configured; a configured camera that is not streaming states its reason on the picture (decision 2 as amended) and the Camera control (F-34) is the way back to terrain | `YonderCockpit.vue`; R-FLT-09, camera-fallback component test. Terrain coverage/datum and registration limits remain visible. Operator ratified 2026-09-12 after the whole-branch review |
...
| F-31 | Official controller terrain panel with disabled-by-default reviewed policy, fixed official source, manual/verified-mission area preview, disarmed preparation, immutable persistent objects, pin/delete/cancel controls and explicit read-only controller refresh; keep coverage/partial/nodata, replies sent and controller pending/loaded as separate facts. State operator-managed single-responder ownership without claiming enforced exclusivity or flight readiness | `OfficialTerrainPanel.vue`, `official-terrain-client.mjs`, `official-terrain-state.mjs`, authenticated cockpit terrain routes and daemon-owned terrain runtime; R-FLT-27/28, policy/apply/component/integration tests and [operator workflow](../../cockpit-user-guide.md#prepare-official-terrain-for-the-controller). Direct FC ports can bypass Yonder; target-board storage and controller acceptance remain pending |
| F-32 | The selected camera's configured preview fills the attitude scene edge to edge with the screen-fixed instruments over it; none of the picture's own toolbar, badges, thumbnail strip or capture controls appears inside the PFD, and no control there starts or stops a stream; one line of reason at the foot of the picture whenever there is one, with the autoplay resume control, and nothing when the picture is fine (decision 2 as amended 2026-09-12; the line is drawn in the full scene, which is the box that can show it whole); no painted split behind the picture; the attitude line stays available over the picture with a switch in PFD settings | Specified in [the camera in the flight display](../../superpowers/specs/2026-09-11-flight-camera-in-pfd-design.md); `YonderPicture.vue` (`scene` prop), `YonderCockpit.vue`, `PrimaryFlightDisplay.vue`, `cockpit.css`; R-FLT-29, R-FLT-09, R-VID-13, `camera-scene.component.test.ts`. Render `instrument-library/flight.camera.full.night.png`; captured live at [`camera-full.png`](../../images/cockpit/camera-full.png). K-68 closed; hardware line pending |
| F-33 | A camera window, home in the top left of the scene, movable by its header and resizable from its corner, place and size remembered locally and reset from PFD settings; synthetic terrain fills the scene behind it; its header carries the stale age and a maximize control, which works because the header's drag gesture yields to controls inside it; a window with no picture carries the cockpit's own unavailable mark — `InstrumentGauge.vue`'s red cross in #ef5a53 with one line beneath it — never over a held frame, and the resume control stays for the autoplay-blocked case (decision 2's amendment, settled 2026-09-12) | Same spec; `CameraWindow.vue`, `camera-view.mjs`, `pfd-controls.mjs`, `PfdControlPanel.vue`, `YonderCockpit.vue`; R-FLT-29, R-FLT-25, `camera-view.test.ts`, `camera-window.component.test.ts`. Render `instrument-library/flight.camera.window.night.png` (composite); captured live at [`camera-window.png`](../../images/cockpit/camera-window.png) |
| F-34 | One Camera button in the top row beside full screen, with a camera-and-arrows glyph, the same height as its neighbours, reading the current state (full or window) and flipping it; a Camera row in the cockpit overflow menu carrying the same state and action for the widths where the top row folds away; both move the same one stored setting as the Background chooser, which survives a reload; disabled and reading unavailable when no camera is selected; never starts or stops a stream | Same spec; `YonderCockpit.vue`, `cockpit-chrome.css`, `cockpit.css`; R-FLT-29, R-FLT-25, R-CMD-04, `camera-view.test.ts`, `camera-window.component.test.ts`, guide-measured height. Render `instrument-library/flight.camera.toggle.night.png` and its zoom |
| F-35 | Height above ground and the forward-clearance forecast keep reporting while the camera fills the scene; a stale picture keeps its hatch and desaturation in the scene and shows its age in the footer label or the window header | Same spec; `TerrainVision.vue` (`draw` prop), `PrimaryFlightDisplay.vue` (`staleLabel`), `CameraWindow.vue`; R-FLT-29, R-FLT-08, R-VID-03, `camera-scene.component.test.ts`, `camera-window.component.test.ts`. Evaluation does not depend on the terrain renderer drawing |
