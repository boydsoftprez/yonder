...
## M9 · R-UI-20 is violated in a committed capture, in the direction the last task flagged
...
R-UI-20: *"an operator must be able to tell this camera cannot from this page failed."* Here
...
outside it. That closes R-UI-20 without building four controls, and makes the gap visible to the
...
RATE requests `/video/<id>/whep` for a path that does not exist and gets a 404. R-UI-20's own
...
## M9 · R-UI-20 violated in a committed capture — **ADDRESSED**
