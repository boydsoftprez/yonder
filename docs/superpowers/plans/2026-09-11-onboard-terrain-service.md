...
- Work in the user's selected current-main checkout; preserve unrelated untracked work. Use signed, DCO-signed commits. Do not push or deploy as a side effect of tests.
...
- [ ] Pin official terraingen and exact ArduPilot references and inspect current download URL, content encoding, DAT structure/version/CRC and upstream interpolation. Record attribution and hashes. Do not execute downloaded pickle files.
- [ ] Acquire a bounded real official object; extract small fixtures with source coordinates, offsets, format version and independent expected values. Include negative coordinates, overlap/boundary, old-version rejection and negative valid elevation. Generate corrupt/nodata fixtures explicitly from valid fixtures and document mutations.
- [ ] Confirm whether one-degree DAT objects require sparse streaming and how requested origins locate blocks. Choose the upstream-preserving representation permitted by the spec; if DAT cannot support bounded deterministic serving, stop only the dependent format task and return the material design decision.
- [ ] Inspect `/var/lib/yonder` mount on the Radxa if reachable. Collect findmnt/statfs evidence and filesystem persistence without altering storage or restarting the board automatically. The reachable board confirms ext4 persistence but insufficient state-volume headroom under the default reserve; record that refusal and design production admission from mountinfo plus a writable-directory probe.
- [ ] Record expected whole-object and transient storage costs and reject downloads exceeding limits before publication.
...
**Requirements:** R-FLT-27/28, R-FLT-05/08. **Dependencies:** Task 1 format contract.
**Owned files:** `official/types.ts`, `official/dat.ts`, `official/dat.test.ts`, `official/grid.ts`, `official/grid.test.ts` and fixture tests.
**Interfaces:** validated immutable block metadata and heights; `decodeTerrainBlock(bytes, expected)`; request-to-block lookup and `sampleAt` matching the pinned AP_Terrain interpolation. No HTTP, filesystem traversal, network sends or configuration writes in the codec.

- [ ] Establish regression fixtures for every accepted format/version, CRC and coordinate identity before enabling decode.
- [ ] Implement bounded offset/length/version/bitmap/spacing/coordinate validation. Reject old-data version, impossible bitmaps, incomplete requested subgrids and wrong object identity. Valid elevation zero and negative elevations remain valid.
- [ ] Implement 4×4 requested subgrid extraction and source-key mapping, including overlapping block edges. Preserve the exact incoming request origin in replies.
- [ ] Implement official point sampling with explicit missing data and datum evidence. Match upstream numerical behavior, including signed-coordinate rounding and grid-edge interpolation.

**Acceptance:** bit0 and bit55 return the correct 16 values; big-endian/little-endian mistakes fail fixtures; corrupt CRC, short input, unsupported versions/spacings and nodata never yield height data; boundary control points match independent reference. **Verification:** `npm run test -w yonder-core -- src/terrain/official/dat.test.ts src/terrain/official/grid.test.ts --maxWorkers=4 --minWorkers=1`. Worker eligible after Task 1; no shared-file edits outside ownership.
...
- [ ] Resolve root beneath `/var/lib/yonder/terrain` (injected temp directory only in tests), verify persistent writable filesystem, reject tmpfs/ramfs/overlay-only and read-only paths, and preserve symlink restrictions.
- [ ] Stream downloads/decompression under independent compressed/decoded/staging/reserve limits. Validate before fsync/rename publication. Retry transient failures at most twice with bounded backoff; honor Retry-After within job lifetime; no retries on format rejection.
- [ ] Save versioned manifests and areas with durable atomic replacement. Scan bounded metadata on restart, ignore/reclaim staging, and recover only complete committed objects. Never persist high-frequency counters.
- [ ] Deduplicate immutable objects; pins and active reads prevent deletion; quota failure leaves prior generations usable. Recheck free bytes during writing, accounting for other writers such as recording.
- [ ] Implement cancellation and disarmed/fresh-context pauses without publishing partial data. Revalidate preview context and source generation before starting and publishing the area manifest.
...
- [ ] Enumerate complete route corridors and loiter extents plus home/rally return corridors using a reviewed positive buffer and bounded geometry work.
- [ ] Handle geographic degree boundaries and longitude wrap explicitly. Refuse oversized/unbounded geometry rather than truncating it into apparently complete coverage.
- [ ] Detect unsupported jump/return/landing geometry, missing home and unknown rally state; preserve an incomplete preview with actionable reasons. Do not equate unknown rally state with zero rally points.
- [ ] Bind selected mission/home/rally identities and source revision into preview token; mission changes make coverage stale, never mutate waypoint altitude values.
...
- [ ] Decode requests through existing CRC/envelope validation and observe all autopilot heartbeats to detect ambiguity. Require a single admitted FC, fresh identity and matching spacing/configuration/capability evidence.
- [ ] Request/observe TERRAIN_ENABLE, TERRAIN_OPTIONS and TERRAIN_SPACING through an explicit refresh action if not already available. No parameter writes in responder. Unknown compatibility remains unavailable with refresh guidance.
- [ ] Implement queue deduplication, expiry, cancellation, byte/frame pacing and busy priority; recheck identity, policy and link immediately before send.
- [ ] Encode requested complete subgrids with exact origin/spacing/bit. Missing tile produces status only. Ignore out-of-range bits, coordinates, unrelated sources and incompatible packets.
- [ ] Track TERRAIN_REPORT separately from transmitted counters; expire on stale heartbeat/reboot/reconnect and show reported pending/loaded without extrapolating mission readiness.
...
- [ ] Add default-disabled policy and schema/default generation; integrate runtime render with rollback to the previous policy. No network reconfiguration is introduced by terrain.
- [ ] Assemble optional runtime once, route packets after VehicleService, and close jobs/timers/readers on shutdown. Faults keep console and telemetry alive.
- [ ] Add authenticated routes with server-derived operator context, idempotent job operations, strict limits and no arbitrary source paths/URLs. Read-only samples need authentication but do not require disarmed state.
- [ ] Make capabilities, storage failures and missing prerequisites explicit; show source/generation in responses. Prevent prepare when armed, stale or changed vehicle/mission context.
...
- [ ] Add terrain-service client and independent slow-status polling, canceled on component teardown; no aircraft refresh on page open.
- [ ] Add Map, terrain & data preparation view: manual map area or mission, editable buffer, preview overlay, source/space summary, explicit Prepare/Cancel, areas and pin/delete, service enablement via configuration flow, explicit controller refresh.
- [ ] Present coverage/service/controller states independently, including paused/stale/incompatible/missing data; add compact PFD entry. Keep display source and detailed terrain controls intact.
- [ ] Route flight-related estimated AGL and target terrain-datum checks through official samples with source/datum validity. Preserve existing mission values and detailed surface visualization; unknown source/datum fails closed.
- [ ] Add authenticated browser scenarios for prepare success/cancel/error, stale revision, quota failure, controller reboot and palette/layout usability.
...
- [ ] Add R-FLT-27/28 and update architecture/configuration/blueprint and command-role documentation alongside implementation.
- [ ] Run simulated-controller end-to-end tests through daemon/router: actual prepared official fixture, browser closed, upstream unavailable, repeated/rebooted/partial requests, concurrent command operation and numerical terrain result comparison. Do not simulate success by directly calling store-only methods.
- [ ] Perform one consolidated independent review of requirements, storage durability, packet isolation and command ownership. Correct material findings and rerun only affected checks before the final combined gate.
...
- Next: obtain operator decision on the discovered DAT nodata ambiguity, update the format-dependent tasks accordingly, then continue through yonder-cost-aware-execution. Hardware storage qualification remains separate.
