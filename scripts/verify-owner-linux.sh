...
set -euo pipefail
[[ $# == 0 && -f CLAUDE.md && -x vendor/node/bin/node ]]
...
docker run --rm --platform linux/arm64 -v "$PWD:/source:ro" -w /source \
    ...
    /bin/bash -c '
        ...
        apt-get install -y --no-install-recommends whois sudo openssh-server sshpass >/dev/null
        ...
        /source/vendor/node/bin/node /lab/core/scripts/verify-owner-linux.mjs
    '
